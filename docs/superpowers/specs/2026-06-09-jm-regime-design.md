...
3. Repeat until `S` unchanged OR `(prev_J − J) < tol` OR `iter == max_iter`:
   - **DP/E-step:** `S = viterbi_argmin(loss_mx, penalty_mx)` (§3.3).
   - **M-step:** for each k, `θ_k = mean(y[S==k])`; if empty → **reseed** (§3.5).
   - recompute `loss_mx`, `J`.
   Objective non-increasing each step (Bemporad 2018 Alg.1) → finite termination to a local optimum.
...
If state k is unused after the DP-step, set `θ_k = y[argmax_t(per-point assigned loss)]` (farthest-point, lowest-index tie-break). Deterministic, never NaN.
...
Replace hard `s_t` with a probability vector on a **fixed discretized simplex grid** `C = discretize_simplex(K, grid_size=0.05)` (all w with components in multiples of `grid_size` summing to 1; ~231 candidates at K=3). Per-day loss row = `loss_mx @ C.T`; transition penalty = `λ·‖c_i − c_j‖₁²` between grid vectors (Aydinhan et al. 2024 L1-squared); **same forward DP** over the grid. `values[e].argmin()` picks the grid vector → soft `(prob_risk_off, prob_transitional, prob_risk_on)` (reordered by perm). Discrete path → one-hot probs, `confidence=1.0`. Behind the `jm_continuous` flag; Shu-Mulvey found no 0/1-allocation edge, so CJM exists here only to feed the stacked-area figure (HMM-parity), not to change labels.

### 3.8 `walk_forward` (mirrors `regime_hmm.walk_forward`, regime_hmm.py:114-176)
`r = jm_min_history_days`; while `r < n`: `block_end = min(r+refit, n)`; fit `Θ`+scaler on `clean.iloc[:r]` (n_init restarts); if converged → `last_good=(Θ,scaler,perm)`, append refit_date; `used = fit if converged else last_good`; emit online states for `[r,block_end)` via §3.6; `r = block_end`. **Identical** NaN-drop/reindex/cold_start handling and the same return-dict keys (§4.1). Re-canonicalize `perm = argsort(centroid means)` ascending → `(Risk-off, Transitional, Risk-on)` **every refit** (FJ2.2; defeats label-permutation symmetry exactly as regime_hmm.py:85 sorts EM states by mean).
...
| `jm_max_iter` | int | `30` | coordinate-descent cap |
| `jm_random_seed` | int | `0` | determinism seed (PCG64) |
...
- **Float/BLAS:** D=1 ⇒ scalar adds/means (no BLAS matmul) → reduction-order nondeterminism negligible in v1. For CJM grid matmul / D>1 follow-on, pin BLAS threads in the run harness. CSV writer reuses `_write_regime_hmm` pattern; if any last-ULP drift appears, round probs to 10 decimals before melt.
