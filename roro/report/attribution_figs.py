...
from roro.report.figures import band_shapes
...
def _require_rows(df: pd.DataFrame | None, name: str) -> pd.DataFrame:
    if df is None or df.empty:
        raise ReportInputError(f"attribution figure needs rows in {name}; got none")
    return df
...
def attribution_bars(bundle: DataBundle, *, top_n: int = _TOP_N_DEFAULT) -> go.Figure:
    ...
    level = _require_rows(bundle.attribution_level, "attribution.csv")
...
def attribution_waterfall(bundle: DataBundle) -> go.Figure:
    ...
    delta = _require_rows(bundle.attribution_delta, "attribution_delta.csv")
    keys = sorted(
...
def concentration_timeseries(bundle: DataBundle) -> go.Figure:
    ...
    concentration = _require_rows(bundle.concentration, "concentration.csv")
    conc = concentration[concentration["weighting"] == "cap"]
    present = set(conc["cut"])
    cuts = [c for c in bundle.seg_tercile.columns if c in present]
    if not cuts:
        raise ReportInputError("concentration.csv has no cut present in regimes.csv")
    ...
    buttons: list[dict[str, object]] = [
        {
            ...
            "args": [
                ...
                {"title": f"Concentration of the slope — {c}",
                 "shapes": band_shapes(bundle.seg_tercile[c], smooth=True)},
            ],
        }
        for c in cuts
    ]
    ...
    layout.shapes = band_shapes(bundle.seg_tercile[default], smooth=True)
    return go.Figure(data=traces, layout=layout)
...
def pc1_loadings_bars(bundle: DataBundle) -> go.Figure:
    ...
    pc1 = _require_rows(bundle.attribution_pc1, "attribution_pc1.csv")
